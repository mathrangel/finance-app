...
module.exports = {
  ...
  async login(req, res, next) {
    try {
        ...
        const user = await User.get().select('password', 'id', 'name').where({ email: email }).first()

        const validatePassword = await bcrypt.compare(password, user.password)
        if (!validatePassword) res.status(400).send({ msg: 'A Senha inserida está incorreta.' })
        
        const token = jwt.sign({ id: user.id, email, name: user.name  }, process.env.JWTWEBTOKEN, { expiresIn: '1h' })
